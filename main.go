// Пакет main реализует программный модуль фитнес-трекера, который обрабатывает
// данные для трех видов тренировок: бега, ходьбы и плавания
//
// Этот модуль рассчитывает результаты тренировки и выводит информационное
...
func (r Running) Calories() float64 {
	s := ((CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / MInKm * float64(r.Duration.Hours()) * MinInHours)
	return s
}
...
func (w Walking) Calories() float64 {
	...
	s := ((CaloriesWeightMultiplier*w.Weight + (math.Pow(w.meanSpeed(), 2)*KmHInMsec/w.Height)*CaloriesSpeedHeightMultiplier*w.Weight) * float64(w.Duration.Hours()) * MinInHours)
	return s
}
...
func (s Swimming) meanSpeed() float64 {
	if float64(s.Duration.Hours()) <= 0 {
		return 0
	}
	g := float64(s.LengthPool) * float64(s.CountPool) / MInKm / float64(s.Duration.Hours())
	return g
}
...
func (s Swimming) Calories() float64 {
	g := (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * float64(s.Duration.Hours())
	return g
}
...
func ReadData(training CaloriesCalculator) string {
	calories := training.Calories() // количество затраченных калорий
	info := training.TrainingInfo() // информация о тренировке
	info.Calories = calories        // добавление полученных калорий в структуру с информацией о тренировке
